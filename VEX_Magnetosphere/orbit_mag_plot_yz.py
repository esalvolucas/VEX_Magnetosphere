...
def orbit_mag_plot_yz(table):
    ...
    ax.plot(table['YSC'],table['ZSC'], color = 'c')
    ax.set(xlabel='VSO Y', ylabel='VSO Z')
    
    # Get a list of the overall magnetic field magnitudes for the desired times
    # Values from table
    overall_mag = table['Bx'].values
    # Now get a list
    o_mags = list()
    for i in mag_line_i:
    ...
    for i, item in enumerate(mag_line_i):
        ...
        color = rgb2hex(r, g, b)
        # Plot the thing
        bplot = ax.plot([table['YSC'][item], table['YSC'][item]+scale*table['By'][item]],
...
def dual_half_circle(center, radius, angle=0, ax=None, colors=('w','w')):
    """
